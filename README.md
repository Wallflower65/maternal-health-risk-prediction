...
from routine antenatal clinical indicators such as age, blood pressure, blood sugar, body
temperature, and heart rate. This project explores how vital signs collected at a standard
...
## Motivation
This project focuses on predicting maternal and neonatal risk using routine data from South African community health centres. Before working with real facility or PPIP data, I wanted to build and test the core modelling approach; feature selection, classification, and evaluation, on a clean, well-understood dataset structured the same way routine antenatal data is typically captured.
...
- Body temperature
- Heart rate
- Risk level (low / mid / high) which is my target variable

*Quick note on the data: I generated this dataset myself rather than pulling the real UCI version, but I built it to mirror real clinical relationships, like how elevated blood sugar and systolic BP tend to push risk classification higher, which lines up with what we know about gestational diabetes and preeclampsia. I designed the pipeline so it'll run on the real UCI dataset or actual facility data without needing to change anything, which is the next thing I want to test.*
...
dominated the model's decisions which are both are measurements already taken at every antenatal
